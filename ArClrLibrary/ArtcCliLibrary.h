...
#include "ArtcCliClientRoleOptions.h"
...
namespace ArtcCliLibrary
{
	...
	public:
		...
		int initialize(CliRtcEngineContext^ context);
		void release();
		int setChannelProfile(ChannelProfile profile);
		int setClientRole(ClientRoleType role);
		int setClientRole(ClientRoleType role, ArtcCliClientRoleOptions options);
		int joinChannel(String^ token, String^ channelName, String^ channelInfo, String^ uid);
		int joinChannel(String^ token, String^ channelName, String^ channelInfo, String^ uid, CliChannelMediaOptions^ options);
		int switchChannel(String^ token, String^ channelId);
		int switchChannel(String^ token, String^ channelId, CliChannelMediaOptions^ options);
		int leaveChannel();
		...
		int enableVideo();
		int disableVideo();
		int setVideoEncoderConfiguration(CliVideoEncoderConfiguration^ config);
		int setupLocalVideo(IntPtr view, int renderMode, String^ uid);
		int setupLocalVideo(IntPtr view, int renderMode, String^ uid, EnumVideoMirrorModeType mt);
		int setupRemoteVideo(IntPtr view, int renderMode, String^ uid);
		int setupRemoteVideo(IntPtr view, int renderMode, String^ uid, EnumVideoMirrorModeType mt);
		int setLocalRenderMode(EnumRenderModeType mode);
		int setLocalRenderMode(EnumRenderModeType mode, EnumVideoMirrorModeType mt);
		int setRemoteRenderMode(String^ uid, EnumRenderModeType mode);
		int setRemoteRenderMode(String^ uid, EnumRenderModeType mode, EnumVideoMirrorModeType mt);
		int startPreview();
		int stopPreview();
		...
		//视频前处理及后处理
		int setBeautyEffectOptions(bool enabled, CliBeautyOptions options); //在3.0上不适配macos, 在2.9.1上只适配iOS与Android,所以不添加

		//todo: 多频道管理
		ArtcCliChannel^ createChannel(String^ channelId);

		//屏幕共享
		//startScreenCaptureByDisplayId 只在macos上有效,所以不添加
		int startScreenCaptureByScreenRect(CliRectangle^ screenRect, CliRectangle^ regionRect, CliScreenCaptureParameters^ params);
		int startScreenCaptureByWindowId(IntPtr windowId, CliRectangle^ regionRect, CliScreenCaptureParameters^ params);
		int setScreenCaptureContentHint(EnumVideoContentHint hint);
		int updateScreenCaptureParameters(CliScreenCaptureParameters^ params);
		int updateScreenCaptureRegion(CliRectangle^ rect);
		...
		int setRemoteVideoStreamType(String^ uid, EnumRemoteVideoStreamType type);
		...
		int startLastmileProbeTest(CliLastmileProbeConfig^ config);
		int stopLastmileProbeTest();

		//通话前网络测试事件
		onLastmileQuality^ onLastmileQuality;
		Action<CliLastmileProbeResult^>^ onLastmileProbeResult;

		//自定义视频模块
		bool setVideoSource(CliVideoSource^ source);

		//音视频自采集
		int setExternalVideoSource(bool enabled, bool useTexture);
		int pushVideoFrame(CliExternalVideoFrame^ frame);
		int setExternalAudioSource(bool enabled, int sampleRate, int channels);
		int pushAudioFrame(CliAudioFrame^ frame);

		//音视频自渲染
		int setExternalAudioSink(bool enabled, int sampleRate, int channels);
		int pullAudioFrame([Out] CliAudioFrame^% frame);
		...
		//直播水印
		int addVideoWatermark(String^ url, CliWatermarkOptions^ options);
		int clearVideoWatermarks();

		//加密
		int enableEncryption(bool enabled, CliEncryptionConfig^ config);
		...
		ArtcCliAudioDeviceManager^ getAudioDeviceManager();
		ArtcCliVideoDeviceManager^ getVideoDeviceManager();
		...
		int createDataStream(int% id, CliDataStreamConfig config);
		...
		int setCameraCapturerConfiguration(CliCameraCaptureConfiguration^ config);
		...
		int startScreenCapture(IntPtr windowId, int captureFreq, CliRect^ rect, int bitrate);
	...
	private:
		...
		void NativeOnAudioVolumeIndication(const ar::rtc::AudioVolumeInfo* speakers, unsigned int speakerNumber, int totalVolume);
		void NativeOnLeaveChannel(const ar::rtc::RtcStats& stats);
		void NativeOnRtcStats(const ar::rtc::RtcStats& stats);
		...
		void NativeOnLocalVideoStats(const ar::rtc::LocalVideoStats& stats);
		void NativeOnRemoteVideoStats(const ar::rtc::RemoteVideoStats& stats);
		...
		bool NativeOnRecordAudioFrame(ar::media::IAudioFrameObserver::AudioFrame& frame);
		bool NativeOnPlaybackAudioFrame(ar::media::IAudioFrameObserver::AudioFrame& frame);
		bool NativeOnPlaybackAudioFrameBeforeMixing(uid_t uid, ar::media::IAudioFrameObserver::AudioFrame& frame);
		bool NativeOnMixedAudioFrame(ar::media::IAudioFrameObserver::AudioFrame& frame);

		bool NativeOnCaptureVideoFrame(ar::media::IVideoFrameObserver::VideoFrame& frame);
		bool NativeOnPreEncodeVideoFrame(ar::media::IVideoFrameObserver::VideoFrame& frame);
		bool NativeOnGetSmoothRenderingEnabled();
		bool NativeOnRenderVideoFrame(uid_t uid, ar::media::IVideoFrameObserver::VideoFrame& frame);
		bool NativeOnRenderVideoFrameEx(const char* channelId,uid_t uid, ar::media::IVideoFrameObserver::VideoFrame& frame);
		...
		void NativeOnNetworkTypeChanged(ar::rtc::NETWORK_TYPE type);
		void NativeOnLocalAudioStateChanged(ar::rtc::LOCAL_AUDIO_STREAM_STATE state, ar::rtc::LOCAL_AUDIO_STREAM_ERROR error);
		void NativeOnLocalVideoStateChanged(ar::rtc::LOCAL_VIDEO_STREAM_STATE state, ar::rtc::LOCAL_VIDEO_STREAM_ERROR error);
